...
function sendmail(){
    ...
    $foldercontent = get-childitem "$selectedDirectory"
            ForEach($item in $foldercontent){

                $ol = New-Object -comObject Outlook.Application;
                $mail = $ol.CreateItem(0);
                $Mail.Recipients.Add("vladyslav.bohachev@ctj-online.de");
                $Mail.Recipients.Add("###");
                $Mail.Subject = "Lieferschein Nr:"+$item.Name;
                $Mail.Body = get-content  -Path "C:\Users\bohachev\Desktop\dmk\body.txt" | Out-String;
